...
from ems.decorators import admin_only, admin_hr_only
from poll.forms import PollForm, ChoiceForm
from poll.models import *

class PollView(View):
    decorators = [login_required, admin_hr_only]
...
def poll(request, id=None):
    ...
    if request.method == 'POST':
        ...
        if ret:
            return HttpResponse("Your vote is successfully saved")
        else:
            return HttpResponse("Your vote is not successfully saved")
        #  return render(request, 'polls/poll.html', context)

def add_polls(request):
    return HttpResponse("Add Pole")
